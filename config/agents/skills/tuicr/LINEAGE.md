...
- `bin/tmux-tuicr` is a separate personal tmux hotkey that jumps to (or opens) a
  `tuicr` pane in the current session. It is not part of this skill; the
  upstream `tuicr-wrapper.sh` here is the agent-facing launcher the skill
  documents, and the two are intentionally distinct.
