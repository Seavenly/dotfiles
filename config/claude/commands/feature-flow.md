...
```
- Record the review so the `tuicr` tmux hotkey (`prefix + r`) can reopen it later: `tuicr-reviews add --repo <repo> --worktree <wt> --base <base> --branch <branch> --slug <slug>` (idempotent; upserts by worktree, and stale entries self-prune once the worktree is removed). Skip only for stuck/RE_PLAN runs that produced no reviewable branch.
