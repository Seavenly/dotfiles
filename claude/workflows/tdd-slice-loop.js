export const meta = {
  name: 'tdd-slice-loop',
  description: 'Shared TDD inner loop (launched as a sub-workflow by feature-flow-run and spike-flow-run, not run directly): per slice, tester → implementer → independent test gate with retries. nonTestable slices skip the tester; tester output is sanity-checked; handoffs fold into the run journal.',
}
...
//     for nonTestable slices these ARE the slice's verification
//   slices: [{ title, behavior, testIdea?, nonTestable?, fixDirection?, evidence? }]
...
for (let i = 0; i < a.slices.length; i++) {
  ...
  // ── Tester (skipped for nonTestable slices) ──
  let testPath = null
  if (!s.nonTestable) {
  ...
  while (true) {
    const retryNote = retries ? `\nRetry ${retries}/${a.maxSliceRetries}. ${testPath ? 'Test still fails' : 'Suite still fails'}:\n${lastFail}\nRevise.${testPath ? ' Do NOT modify the test.' : ''}` : ''
    ...
    const implPrompt = s.nonTestable
      ? `${inWorktree}\nSlice: ${s.title}. ${s.behavior}\nThis slice has no behavioral unit-test surface (infra/config). Implement it minimally.${s.fixDirection ? ` Fix direction: ${s.fixDirection}.` : ''}${obsNote} Do not commit — the gate commits on green.\nNotes so far:\n${renderNotes()}${retryNote}\n${readRefs} Return your ### Handoff.`
    ...
    const verifyClause = (a.verifyCmds && a.verifyCmds.length)
      ? ` Then run the verification command(s) ${a.verifyCmds.map((c) => `\`${c}\``).join(', ')} — each must also succeed (for infra/config slices with no test surface, these ARE the gate; treat a failed/erroring preview or an unexpected replace/delete of protected resources as a failure).`
      : ''
    const g = await agent(
