...
test("delegate failure observations preserve their strict operator fields", () => {
  ...
  assert.deepEqual(sanitizeProviderObservation(observation), observation);
  assert.equal(sanitizeProviderObservation({
    ...observation,
    retryable: "false",
  }), null);
});
...
test("typed delegate failures survive durable persistence, replay, and projection", async (t) => {
  const authorityDirectory = await mkdtemp(join(tmpdir(), "flow-delegate-failure-replay-"));
  t.after(() => rm(authorityDirectory, { recursive: true, force: true }));
  const authority = createDurableRunAuthority({
    authorityDirectory,
    hostIdentityAdapter: fixedHostIdentity("boot-a", "process-delegate-failure"),
  });
  ...
  const intent = runtime.command(execution).effect_intents[0];
  const failure = {
    schema: "flow.delegate-failure-observation/v1",
    code: "invalid_envelope",
    stage: "delegate_effect_materialization",
  ...
  assert.deepEqual(live.effects[0].last_observation.provider_observation, failure);

  authority.close();
  const reopened = createDurableRunAuthority({
    authorityDirectory,
    hostIdentityAdapter: fixedHostIdentity("boot-a", "process-delegate-failure-replay"),
  });
  ...
  assert.deepEqual(
    replayed.effects[0].last_observation.provider_observation,
    failure,
  );
  assert.equal(replayed.effects[0].last_observation.presence, "indeterminate");
});
