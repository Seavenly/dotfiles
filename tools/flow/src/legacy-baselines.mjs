...
export function auditLegacyBaselinesSync({ repositoryRoot, inventoryPath }) {
  ...
  for (const baseline of inventory.baselines) {
    ...
    for (const component of baseline.components ?? []) {
      assertRepositoryPath(repositoryRoot, component.source_path);
      const recorded = resolveGitObjectSync({
        repositoryRoot,
        revision: inventory.source_commit,
        sourcePath: component.source_path,
      });
      const current = resolveGitObjectSync({
        repositoryRoot,
        revision: "HEAD",
        sourcePath: component.source_path,
      });
      const workingTree = resolveWorkingTreeStatusSync({
        repositoryRoot,
        sourcePath: component.source_path,
      });
      components.push({
        source_path: component.source_path,
        expected_git_object: component.git_object,
        recorded_git_object: recorded.gitObject,
        current_git_object: current.gitObject,
        recorded_failure: recorded.failure,
        current_failure: current.failure,
        working_tree_clean: workingTree.clean,
        working_tree_failure: workingTree.failure,
        status: recorded.gitObject === component.git_object &&
            current.gitObject === component.git_object
          ? "passed"
          : "failed",
      });
    }
  ...
  return {
    ...
    status: baselines.every((baseline) => baseline.status === "passed")
      ? "passed"
      : "failed",
    working_tree_clean: baselines.every(({ working_tree_clean }) => working_tree_clean),
    baselines,
  };
}
...
function resolveGitObjectSync({ repositoryRoot, revision, sourcePath }) {
  try {
    ...
    return { gitObject: null, failure: "unresolved_git_object" };
  }
}

function resolveWorkingTreeStatusSync({ repositoryRoot, sourcePath }) {
  try {
    const stdout = execFileSync(
      "git",
      ["status", "--porcelain=v1", "--untracked-files=all", "--", sourcePath],
