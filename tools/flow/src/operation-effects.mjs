...
function safeProviderObservation(value, registration = null, intent = null) {
  ...
  // Reconciliation may return the provider's versioned receipt itself. Keep
  // that exact approved receipt shape available for adoption; only ordinary
  // observations go through the conservative observation envelope.
  return sanitizeProviderEvidence(value, intent);
}
...
export function sanitizeProviderObservation(value, intent = null) {
  if (!isRecord(value)) return null;
  if (value.schema === DELEGATE_FAILURE_OBSERVATION_SCHEMA) {
    return sanitizeDelegateFailureObservation(value);
  }
  return genericSafeProviderObservation(value, intent);
}

function sanitizeDelegateFailureObservation(value) {
  if (typeof value.code !== "string" ||
      !/^[a-z0-9_:-]{1,128}$/u.test(value.code) ||
      value.stage !== "delegate_effect_materialization" ||
