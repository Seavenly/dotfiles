...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME icon.color=$COLOR_FUJI_WHITE label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME icon.color=$COLOR_FUJI_WHITE
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME icon.color=$COLOR_FUJI_WHITE label.drawing=no
    else
        sketchybar --set $NAME icon.color=$COLOR_SPRING_GREEN
    fi
}
